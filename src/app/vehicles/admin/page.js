...
export default function VehicleAdminDashboard() {
  ...
  const getColorClasses = (color) => {
    const colors = {
      blue: { bg: 'bg-gray-500', light: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200' },
      green: { bg: 'bg-gray-500', light: 'bg-green-50', text: 'text-green-600', border: 'border-green-200' },
      orange: { bg: 'bg-gray-500', light: 'bg-orange-50', text: 'text-orange-600', border: 'border-orange-200' },
      red: { bg: 'bg-gray-500', light: 'bg-red-50', text: 'text-red-600', border: 'border-red-200' },
      gray: { bg: 'bg-gray-500', light: 'bg-gray-50', text: 'text-gray-600', border: 'border-gray-200' },
      teal: { bg: 'bg-gray-500', light: 'bg-teal-50', text: 'text-teal-600', border: 'border-teal-200' },
      indigo: { bg: 'bg-gray-500', light: 'bg-indigo-50', text: 'text-indigo-600', border: 'border-indigo-200' },
      emerald: { bg: 'bg-gray-500', light: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-200' },
      purple: { bg: 'bg-gray-500', light: 'bg-purple-50', text: 'text-purple-600', border: 'border-purple-200' }
    };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 text-white p-6 rounded-xl shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-green-100 text-sm mb-1">متاحة</p>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-6 rounded-xl shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-orange-100 text-sm mb-1">قيد الاستخدام</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className={`border-r-4 border-orange-500 bg-orange-50 p-5 rounded-lg`}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-orange-600 mb-1">مشاكل وصيانة</p>
              <p className="text-3xl font-bold text-orange-700">{stats.problemCount}</p>
            </div>
        ...
        <div className={`border-r-4 border-red-500 bg-red-50 p-5 rounded-lg`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-red-700">{stats.stolenCount}</p>
            </div>
        ...
        <div className={`border-r-4 border-gray-500 bg-gray-50 p-5 rounded-lg`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-700">{stats.breakUpCount}</p>
            </div>
          ...
          {quickActions.map((action) => {
            ...
            return (
              <Link key={action.path} href={action.path}>
                <div className={`border ${colors.border} rounded-xl p-5 hover:shadow-lg transition cursor-pointer bg-white`}>
                  <div className="flex items-start justify-between mb-3">
                    <div className={`${colors.light} p-3 rounded-lg`}>
                      <Icon className={colors.text} size={24} />
                    </div>
                    {action.count !== null && (
                      <span className={`px-3 py-1 ${colors.light} ${colors.text} rounded-full text-sm font-bold`}>
                        {action.count}
                      </span>
                    )}
                  </div>
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-gray-600">متاحة</span>
                    <span className="font-medium">{((stats.availableCount / totalVehicles) * 100).toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-green-500 h-2 rounded-full transition-all"
                      style={{ width: `${(stats.availableCount / totalVehicles) * 100}%` }}
                    ></div>
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-medium">{((stats.takenCount / totalVehicles) * 100).toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-blue-500 h-2 rounded-full transition-all"
                      style={{ width: `${(stats.takenCount / totalVehicles) * 100}%` }}
                    ></div>
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-medium">{((stats.problemCount / totalVehicles) * 100).toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-orange-500 h-2 rounded-full transition-all"
                      style={{ width: `${(stats.problemCount / totalVehicles) * 100}%` }}
                    ></div>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className="font-bold text-blue-600">{activeVehicles}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className="font-bold text-gray-600">{stats.breakUpCount}</span>
                </div>
              </div>
            </div>
          </div>
