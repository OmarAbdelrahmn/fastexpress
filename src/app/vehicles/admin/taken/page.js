...
import { Users, Search, Car, MapPin, Calendar, User, Clock, Eye, AlertTriangle, Shield, PackageX } from 'lucide-react';
...
export default function TakenVehiclesPage() {
  ...
  const loadVehicles = async () => {
    setLoading(true);
    try {
      const data = await ApiService.get(`/api/vehicles/taken?statusFilter=${filterStatus}`);
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'Taken': return { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-600', badge: 'bg-blue-600' };
      case 'Problem': return { bg: 'bg-orange-50', border: 'border-orange-200', text: 'text-orange-600', badge: 'bg-orange-600' };
      case 'Stolen': return { bg: 'bg-red-50', border: 'border-red-200', text: 'text-red-600', badge: 'bg-red-600' };
      case 'BreakUp': return { bg: 'bg-gray-50', border: 'border-gray-200', text: 'text-gray-600', badge: 'bg-gray-600' };
      default: return { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-600', badge: 'bg-blue-600' };
    }
  };

  const getStatusLabel = (status) => {
    switch (status) {
      case 'Taken': return 'مستخدمة';
      case 'Problem': return 'مشكلة';
      case 'Stolen': return 'مسروقة';
      case 'BreakUp': return 'خارج الخدمة';
    ...
    <div className="space-y-6">
        ...
        <div className="bg-blue-50 border-r-4 border-blue-500 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-700">{vehiclesData?.takenCount || 0}</p>
            </div>
        ...
        <div className="bg-orange-50 border-r-4 border-orange-500 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-700">{vehiclesData?.problemCount || 0}</p>
            </div>
        ...
        <div className="bg-red-50 border-r-4 border-red-500 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-700">{vehiclesData?.stolenCount || 0}</p>
            </div>
        ...
        <div className="bg-gray-50 border-r-4 border-gray-500 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-700">{vehiclesData?.breakUpCount || 0}</p>
            </div>
        ...
        <div className="bg-purple-50 border-r-4 border-purple-500 p-4 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-700">{vehiclesData?.totalCount || 0}</p>
            </div>
      ...
      <Card>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="البحث برقم اللوحة، اسم السائق، الحالة..."
            value={searchTerm}
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <span className={`px-3 py-1 ${colors.badge} text-white rounded-full text-xs font-medium`}>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                {/* Status Info */}
                <div className={`${getStatusColor(selectedVehicle.statusType).bg} p-4 rounded-lg border-r-4 ${getStatusColor(selectedVehicle.statusType).border.replace('border-', 'border-r-')}`}>
                  <h3 className={`font-bold ${getStatusColor(selectedVehicle.statusType).text} mb-2`}>
                  ...
                  {selectedVehicle.reason && (
                    <p className="text-sm text-gray-700 mt-2"><strong>السبب:</strong> {selectedVehicle.reason}</p>
                  )}
                </div>
                    ...
                    {selectedVehicle.manufacturer && (
                      <>
                        ...
                        <div>
                          ...
                          <p className="font-medium text-gray-800">{selectedVehicle.manufactureYear || 'N/A'}</p>
                        </div>
                      </>
                    )}
                  </div>
                ...
                {selectedVehicle.riderName && selectedVehicle.riderName !== 'N/A' && selectedVehicle.riderName !== 'Unknown' && (
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <h3 className="font-bold text-blue-800 mb-3">معلومات السائق</h3>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <p className="text-blue-600 mb-1">اسم السائق (عربي)</p>
                        <p className="font-medium text-gray-800">{selectedVehicle.riderName}</p>
                      </div>
                      {selectedVehicle.riderNameE && selectedVehicle.riderNameE !== 'N/A' && (
                        <div>
                          <p className="text-blue-600 mb-1">اسم السائق (إنجليزي)</p>
                          <p className="font-medium text-gray-800">{selectedVehicle.riderNameE}</p>
                        </div>
                      )}
                ...
                {selectedVehicle.ownerName && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="font-bold text-gray-800 mb-3">معلومات المالك</h3>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-medium text-gray-800">{selectedVehicle.ownerId || 'غير محدد'}</p>
                      </div>
                    </div>
                  </div>
                )}
