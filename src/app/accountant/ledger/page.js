...
export default function LedgerWorkspacePage({ initialTab = 'accounts' }) {
  ...
  const journalConfigs = {
    create: {
      ...
      fields: [field('legalEntityId', 'الكيان القانوني', 'Legal entity', 'number', { required: true, defaultValue: legalEntityId || '' }), field('branchId', 'الفرع (اختياري)', 'Branch ID (optional)', 'number'), field('transactionDate', 'تاريخ العملية', 'Transaction date', 'date', { required: true }), field('description', 'الوصف', 'Description', 'textarea', { required: true, full: true }), field('currencyCode', 'العملة', 'Currency', 'text', { required: true, defaultValue: 'SAR', dir: 'ltr' }), field('exchangeRate', 'سعر الصرف', 'Exchange rate', 'number', { required: true, defaultValue: 1, min: 0.000001, step: '0.000001' }), field('idempotencyKey', 'مفتاح عدم التكرار', 'Idempotency key', 'text', { required: true, defaultValue: idempotency(), dir: 'ltr', full: true }), field('lines', 'سطور القيد', 'Journal lines', 'lineItems', { required: true, full: true, defaultValue: [{ accountId: '', description: '', debit: '', credit: '' }], columns: [{ name: 'accountId', label: { ar: 'الحساب', en: 'Account' }, type: 'number', required: true }, { name: 'description', label: { ar: 'الوصف', en: 'Description' }, required: true }, { name: 'debit', label: { ar: 'مدين', en: 'Debit' }, type: 'number', step: '0.01' }, { name: 'credit', label: { ar: 'دائن', en: 'Credit' }, type: 'number', step: '0.01' }], })],
