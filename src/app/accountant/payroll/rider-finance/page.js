...
import {
  ActionButton, ApiProblemDetails, DataTable, EmptyState, ErrorState, FormField, LoadingState,
...
export default function RiderFinancePage() {
  ...
  const lookupProfile = async (event) => {
    ...
    catch (requestError) { setActionError(requestError); setProfile(null); }
    finally { setBusy(''); }
  };
  ...
  const inspectItem = async (item) => { setBusy(`item-${item.id}`); setActionError(''); try { setSelectedItem(await accountingApi.payroll.getItem(item.id)); } catch (requestError) { setActionError(requestError); } finally { setBusy(''); } };
  ...
  return <div className="space-y-5" dir={isRtl ? 'rtl' : 'ltr'}>
    ...
    {!legalEntityId ? <EmptyState icon={UserRoundSearch} title={copy.requiredEntity} /> : <>
      ...
      <Panel title={copy.types}>{loading ? <LoadingState /> : error ? <ApiProblemDetails error={error} fallback={copy.loadError} /> : <DataTable columns={typeColumns} data={types} rowKey="id" />}</Panel>
      <Panel title={copy.items}>{loading ? <LoadingState /> : error ? <ApiProblemDetails error={error} fallback={copy.loadError} /> : <DataTable columns={itemColumns} data={items} rowKey="id" emptyTitle={copy.empty} />}</Panel>
