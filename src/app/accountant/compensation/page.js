...
const optionLabel = (name) => name.replace(/([a-z])([A-Z])/g, '$1 $2');
...
export default function CompensationPoliciesPage() {
  ...
  const load = useCallback(async () => {
    if (!legalEntityId) { setPolicies([]); setLoading(false); return; }
    setLoading(true);
    setError('');
    try {
      const payload = await callApi(accountingApi.compensation, ['listPolicies'], {
      ...
      {!legalEntityId ? <EmptyState icon={Calculator} title={copy.requiredEntity} /> : <>
        ...
        <Panel title={copy.createTitle} description={copy.createDescription}>
          <form className="space-y-5" onSubmit={createPolicy}>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
              <FormField label={copy.platform} required><input inputMode="numeric" required value={form.platformAccountId} onChange={(event) => setForm((current) => ({ ...current, platformAccountId: event.target.value }))} /></FormField>
            ...
            {form.rules.map((rule, index) => (
              <fieldset key={index} className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                  ...
                  <FormField label={copy.template} required><select value={rule.template} onChange={(event) => updateRule(index, 'template', event.target.value)}>{RULE_TEMPLATES.slice(1).map((name, optionIndex) => <option key={name} value={optionIndex + 1}>{optionLabel(name)}</option>)}</select></FormField>
                  <FormField label={copy.component} required><select value={rule.componentType} onChange={(event) => updateRule(index, 'componentType', event.target.value)}>{COMPONENT_TYPES.slice(1).map((name, optionIndex) => <option key={name} value={optionIndex + 1}>{optionLabel(name)}</option>)}</select></FormField>
                  ...
                  <FormField label={copy.stacking} required><select value={rule.stackingMode} onChange={(event) => updateRule(index, 'stackingMode', event.target.value)}>{STACKING_MODES.slice(1).map((name, optionIndex) => <option key={name} value={optionIndex + 1}>{optionLabel(name)}</option>)}</select></FormField>
        ...
        <Panel title={copy.register} description={copy.registerDescription}>
          <div className="mb-4 grid gap-3 sm:grid-cols-2 lg:max-w-2xl">
            ...
            <select className={controlClass} value={filters.status} onChange={(event) => setFilters((current) => ({ ...current, status: event.target.value, pageNumber: 1 }))}><option value="">{copy.allStatuses}</option>{POLICY_STATUSES.slice(1).map((status, index) => <option key={status} value={index + 1}>{status}</option>)}</select>
          </div>
