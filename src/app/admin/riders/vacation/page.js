...
function MemberNotes({ item, compact = false }) { if (!item?.memberNotes) return compact ? <span className="text-xs text-slate-400">—</span> : null; return <div className={compact ? 'text-xs text-slate-600' : 'rounded-lg bg-slate-50 p-3 text-sm text-slate-700'}><p className={compact ? 'whitespace-pre-wrap break-words' : 'mb-1 text-xs font-semibold text-slate-500'}>{compact ? item.memberNotes : 'ملاحظة العضو'}</p>{!compact && <p className="whitespace-pre-wrap break-words">{item.memberNotes}</p>}</div>; }
