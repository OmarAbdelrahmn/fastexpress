...
function RequestCard({ item, actions }) { return <article className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-2"><div><h3 className="font-bold text-slate-900">{displayRider(item)}</h3><p className="mt-1 text-xs text-slate-500">{dateValue(item.startDate)} — {dateValue(item.endDate)}</p></div><Status status={item.status} stage={item.stage} /></div><div className="mt-3"><MemberNotes item={item} /></div><div className="mt-4 flex flex-wrap gap-2">{actions}</div></article>; }
