...
export default function ShiftComparisonsPage() {
  ...
  const loadPendingComparisons = async () => {
    ...
    try {
      ...
      if (error.status === 404) {
        setComparisons(null);
        setMessage({ type: 'info', text: t('shifts.noPendingForDate') });
      } else {
        setMessage({ type: 'error', text: error.message || t('shifts.loadError') });
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateComparisons = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({
          type: 'success',
          text: `${t('shifts.totalComparisons')}: ${result.totalComparisons} (${result.newShifts} ${t('shifts.new')}, ${result.updatedShifts} ${t('shifts.updates')})`
        });
        setUploadFile(null);
      } else {
        setMessage({ type: 'error', text: result.title || t('shifts.importFailed') });
      }
    } catch (error) {
      setMessage({ type: 'error', text: t('shifts.errorDuringImport') });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResolve = async (choice) => {
    if (!confirm(choice === '1' ? t('shifts.confirmKeepExisting') : t('shifts.confirmUseNew'))) return;
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: `${t('shifts.processed')}: ${result.totalResolved}`
      });
      setComparisons(null);
    } catch (error) {
      setMessage({ type: 'error', text: error.message || t('shifts.processingError') });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-blue-50 to-blue-100" dir="rtl">
      <PageHeader
        title={t('shifts.shiftComparison')}
        subtitle={t('shifts.comparisonSubtitle')}
        icon={GitCompare}
      />
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <h3 className="text-lg font-bold text-gray-800 mb-4">{t('shifts.importComparisonFile')}</h3>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-blue-50 border border-blue-200 p-5 rounded-lg">
                <p className="text-blue-600 text-sm mb-1">{t('shifts.totalComparisons')}</p>
                <p className="text-3xl font-bold text-blue-700">{comparisons.totalComparisons}</p>
              </div>
              <div className="bg-green-50 border border-green-200 p-5 rounded-lg">
                <p className="text-green-600 text-sm mb-1">{t('shifts.newShifts')}</p>
              ...
              <div className="bg-yellow-50 border border-yellow-200 p-5 rounded-lg">
                <p className="text-yellow-600 text-sm mb-1">{t('shifts.updates')}</p>
            ...
            {/* Resolution Buttons */}
            <div className="bg-white rounded-xl shadow-md p-6">
              <h3 className="text-lg font-bold text-gray-800 mb-4">{t('shifts.comparisonActions')}</h3>
            ...
            <div className="space-y-4">
              <h3 className="text-xl font-bold text-gray-800">{t('shifts.details')} ({comparisons.comparisons?.length || 0})</h3>
                      ...
                      <div className="flex items-center gap-4">
                        {comp.isSubstitution && (
                          <span className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-medium border border-yellow-200">
                            {t('shifts.substitute')} ({t('shifts.original')}: {comp.originalRiderWorkingId})
                          </span>
                        )}
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${comp.oldData.acceptedOrders === null ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-blue-100 text-blue-800 border border-blue-200'
                          }`}>
                          {comp.oldData.acceptedOrders === null ? t('shifts.new') : t('shifts.update')}
                        </span>
                  ...
                  {showDetails[index] && (
                    <div className="p-6 bg-gray-50">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white p-5 rounded-lg border-2 border-gray-300">
                          <h5 className="font-bold text-gray-700 mb-4 flex items-center gap-2">
                            ...
                            {t('shifts.currentData')}
                          </h5>
                          {comp.oldData.acceptedOrders === null ? (
                            <p className="text-gray-500 text-center py-8">{t('shifts.noPreviousData')}</p>
                          ) : (
                            <div className="space-y-3">
                              <div className="flex justify-between">
                                <span className="text-gray-600">{t('shifts.acceptedOrdersLabel')}</span>
                                <span className="font-bold text-gray-800">{comp.oldData.acceptedOrders}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">{t('shifts.rejectedOrdersLabel')}</span>
                                <span className="font-bold text-gray-800">{comp.oldData.rejectedOrders}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">{t('shifts.stackedDeliveriesLabel')}</span>
                                <span className="font-bold text-gray-800">{comp.oldData.stackedDeliveries || 0}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">{t('shifts.workingHoursLabel')}</span>
                                <span className="font-bold text-gray-800">{comp.oldData.workingHours?.toFixed(1)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-gray-600">{t('shifts.statusLabel')}</span>
                      ...
                      {comp.analysis.hasChanges && (
                        <div className={`mt-4 p-4 rounded-lg border ${comp.analysis.recommendation.includes('Improvement') ? 'bg-green-50 border-green-200' :
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <h6 className="font-bold text-gray-800 mb-1">{t('shifts.recommendation')}</h6>
        ...
        {!comparisons && !loading && (
          <div className="bg-white rounded-xl shadow-md p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-600 mb-2">{t('shifts.noPendingComparisons')}</h3>
            <p className="text-gray-500">{t('shifts.uploadToCreateComparison')}</p>
          </div>
        )}
      </div>
    </div>
