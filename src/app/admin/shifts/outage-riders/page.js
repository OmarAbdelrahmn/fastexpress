        ...
        <section className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
              ...
              <input
                type="date"
                value={performanceForm.shiftDate}
                onChange={(e) => setPerformanceForm({ ...performanceForm, shiftDate: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{text.outRiderInfo}</label>
              <select
                value={performanceForm.outRiderInfoId}
                onChange={(e) => setPerformanceForm({ ...performanceForm, outRiderInfoId: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
              >
                <option value="">{text.selectRider}</option>
                {riders.map((rider) => (
                  <option key={rider.id} value={rider.id}>{selectedRiderLabel(rider)}</option>
                ))}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{text.acceptedOrders}</label>
              <input
                type="number"
                min="0"
                value={performanceForm.acceptedOrders}
                onChange={(e) => setPerformanceForm({ ...performanceForm, acceptedOrders: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{text.rejectedOrders}</label>
              <input
                type="number"
                min="0"
                value={performanceForm.rejectedOrders}
                onChange={(e) => setPerformanceForm({ ...performanceForm, rejectedOrders: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">{text.workingHours}</label>
              <input
                type="number"
                min="0"
                step="0.1"
                value={performanceForm.workingHours}
                onChange={(e) => setPerformanceForm({ ...performanceForm, workingHours: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
              />
            </div>
          </div>
          ...
          <div className="mt-5 flex justify-end gap-2">
            ...
            <button
              ...
              className="bg-indigo-600 text-white px-5 py-2 rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2 font-medium"
            >
              {editingPerformanceId ? <Edit size={18} /> : <Plus size={18} />}
              {editingPerformanceId ? text.update : text.create}
            </button>
          </div>
        </section>
