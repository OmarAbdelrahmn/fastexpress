...
export default function KetaRiderHistoryPage() {
    ...
    const fetchReport = async () => {
        ...
        setError(null);
        try {
            // Using RIDER_HISTORY2 for Keeta
            const data = await ApiService.get(API_ENDPOINTS.REPORTS.RIDER_HISTORY2(iqamaNo));
            setReportData(data);


            // Update URL params
            router.push(`/admin/reports/keta/history?iqamaNo=${iqamaNo}`, { scroll: false });
        } catch (err) {
    ...
    return (
        <div >
            <PageHeader
                title={t('reports.riderHistory') + " (كيتا)"}
