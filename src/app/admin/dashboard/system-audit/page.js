...
export default function SystemAuditPage() {
  ...
  const [result, setResult] = useState({ totalCount: 0, page: 1, pageSize: PAGE_SIZE, items: [] });
  ...
  const load = useCallback(async (targetPage = page, activeFilters = filters) => {
    if (!master) return;
    setLoading(true); setError('');
    try {
      const response = await SystemAuditService.list(requestParams(activeFilters, targetPage));
      setResult({ totalCount: response?.totalCount ?? 0, page: response?.page ?? targetPage, pageSize: response?.pageSize ?? PAGE_SIZE, items: Array.isArray(response?.items) ? response.items : [] });
    } catch (requestError) { setError(requestError.message || text.loadError); } finally { setLoading(false); }
  }, [filters, master, page, text.loadError]);
  ...
  return <div className="min-h-screen bg-slate-50 px-4 py-5 sm:px-6 lg:px-10" dir={english ? 'ltr' : 'rtl'}>
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-50 text-blue-700"><CalendarClock size={18} /></div><div><h2 className="font-bold text-slate-900">{result.totalCount.toLocaleString(english ? 'en-GB' : 'ar-SA')} {text.events}</h2><p className="text-xs text-slate-500">{filterCount ? `${filterCount} ${text.active}` : text.recent}</p></div></div><button onClick={() => setDrawerOpen(true)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm font-bold text-slate-700 hover:bg-slate-50"><SlidersHorizontal size={17} />{text.filters}</button></div>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-slate-100 px-5 py-4"><p className="text-sm text-slate-500">{text.page} {result.page} {english ? 'of' : 'من'} {totalPages}</p><div className="flex gap-2"><button disabled={loading || result.page <= 1} onClick={() => { const target = result.page - 1; setPage(target); load(target); }} className="min-h-10 rounded-lg border border-slate-200 px-3 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-45">{text.previous}</button><button disabled={loading || result.page >= totalPages} onClick={() => { const target = result.page + 1; setPage(target); load(target); }} className="min-h-10 rounded-lg border border-slate-200 px-3 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-45">{text.next}</button></div></div>
    </section>
