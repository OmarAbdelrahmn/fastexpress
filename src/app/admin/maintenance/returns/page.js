...
import { Plus, Edit, Trash2, Search, RotateCcw, Filter, List, Eye } from 'lucide-react';
...
export default function ReturnsPage() {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            // Check if there is a delete endpoint for returns, if not, we skip or use a generic one if exists
            // For now, assuming standard CRUD
            await ApiService.delete(`${API_ENDPOINTS.RETURN.LIST}/${id}`);
    ...
    const handleSubmit = async (formData) => {
        try {
            if (editingItem) {
                await ApiService.put(`${API_ENDPOINTS.RETURN.LIST}/${editingItem.id}`, formData);
                showAlert('success', 'تم تحديث البيانات بنجاح');
            } else {
                await ApiService.post(API_ENDPOINTS.RETURN.CREATE, formData);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (row) => `${Number(row.totalAmount || 0).toFixed(2)} ر.س`
        },
        {
            header: 'الإجراءات',
            render: (row) => (
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm">
                <div className="flex gap-4 mb-4">
                    <div className="flex-1">
                        <Input
                            placeholder="بحث عن مرتجع (رقم المرتجع، السبب، المورد)..."
                            value={searchQuery}
            ...
            <Modal
                ...
                title={editingItem ? 'تعديل مرتجع' : 'إضافة مرتجع جديد'}
                size="lg"
            >
                <ReturnForm
