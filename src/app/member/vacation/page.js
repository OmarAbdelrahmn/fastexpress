...
function Status({ status, stage }) {
  ...
  return <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${tone}`}>{status !== null && status !== undefined ? displayStatus(status, locale) : displayStage(stage, locale)}</span>;
}
