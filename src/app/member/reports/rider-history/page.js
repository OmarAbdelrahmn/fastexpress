...
export default function RiderHistoryPage() {
    ...
    const fetchReport = async () => {
        ...
        setError(null);
        try {
            // Using the requested enpoint: api/Member/rider/history?riderIqamaNo=...
            // Assuming API_ENDPOINTS structure allows adding this path or using absolute path if needed.
            // Since I don't see the endpoints file, I'll assume I can construct it or add it.
            // But good practice is to use the constant. I will use a direct string for now if not in constants, 
            // but effectively I should add it to endpoints.js if I could. 
            // For now, I'll just use the string relative to Member base if possible or full path.
            // The ApiService usually handles the base URL.
            const url = `/api/Member/rider/history?riderIqamaNo=${iqamaNo}`;
            const response = await ApiService.get(url);
            setReportData(response);

            // Update URL params
            router.push(`/member/reports/rider-history?iqamaNo=${iqamaNo}`, { scroll: false });
        } catch (err) {
                        ...
                        <div className="relative">
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    // If input is numeric, treat as iqama? Or just clear to force select?
                                    // Better to force select or exact match.
                                    // Let's clear iqamaNo if typing to ensure valid selection
                                    // unless it's a direct paste of full number? 
                                    // The user said "select the riders by name and send the iqamano"
                                    if (!val) setIqamaNo("");
                                }}
                                onFocus={() => setShowDropdown(true)}
                                className="px-4 py-2 pl-10 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all text-gray-700 font-medium shadow-sm w-80"
                            />
