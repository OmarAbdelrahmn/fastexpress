...
const toDateTimeLocal = (value) => {
  if (!value) return new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString().slice(0, 16);
  return new Date(value).toISOString().slice(0, 16);
};

export default function AccountantBillForm({ initialData, suppliers = [], onSubmit, onCancel, isLoading = false }) {
  const [formData, setFormData] = useState({
    supplierId: '',
    invoiceNumber: '',
    invoiceDate: toDateTimeLocal(),
    notes: '',
    items: [EMPTY_ITEM],
  });
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      invoiceDate: toDateTimeLocal(initialData.invoiceDate || initialData.processedAt),
    ...
    setFormData((current) => {
      const items = current.items.map((item, itemIndex) => {
        ...
        if (field === 'itemId' && value) {
          ...
          if (selectedItem?.price !== undefined) nextItem.unitPrice = selectedItem.price;
        }
